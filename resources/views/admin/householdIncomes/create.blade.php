...
<form method="POST" action="{{ route('admin.householdIncomes.store') }}">
		@csrf
...
<br />
		...
		<div class="form-group row">
			...
			<div class="col-md-6">
				<input id="d_o_b" type="date" name="d_o_b" class="form-control" placeholder="Enter your ID number" required />
			</div>
		</div>
		...
		<input style="display: none;" id="indigent_id" type="number" name="indigent_id" class="form-control" value="{{$indigent->id}}" readonly required />
		...
		<a href="{{ route('admin.householdConditions.create', ['id' => $indigent->id]) }}" class="btn btn-secondary">Skip and Continue</a>
	</form>
